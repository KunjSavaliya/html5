'use strict'; {
    ...
    async function LoadScripts(scriptsArr) {
        if (scriptsType ===
            "classic") importScripts(...scriptsArr);
        else if (scriptsArr.length === 1) {
    ...
    async function InitRuntime(data) {
        ...
        if (scriptsType === "module") {
            ...
            for (let [originalUrl, loadUrl] of allProjectScripts) {
                ...
                } else if (originalUrl === "scriptsInEvents.js" || originalUrl.endsWith("/scriptsInEvents.js")) await LoadScripts([loadUrl])
            }
        } else if (projectScripts &&
            projectScripts.length > 0) try {
            await LoadScripts(projectScripts.map(e => e[1]))
        } catch (err) {
            console.error("[Preview] Error loading project scripts: ", err);
            ReportProjectScriptError(projectScripts, scriptsStatus, messagePort);
            return
        }
    ...
    async function ReportProjectScriptError(projectScripts, scriptsStatus, messagePort) {
        let msg;
